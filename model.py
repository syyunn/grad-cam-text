...
class Model(object):
    def build(
        ...
        with tf.name_scope("cnn-layer"):
            self.max_over_time_pooled_layer = self._cnn_layer(
                self.input_data, params["kernels"]
            )
            self.max_over_time_pooled_layers.append(self.max_over_time_pooled_layer)

        with tf.name_scope("concatenated-layer"):
            self.concatenated_layer = tf.concat(
                self.max_over_time_pooled_layers, axis=1
            )

        with tf.name_scope("fc-layer"):
            self.highway_layer = self._fc_layer(
                input_layer=self.concatenated_layer,
                training=(mode == tf.estimator.ModeKeys.TRAIN),
            )

        # logits
        with tf.name_scope("logit-layer"):
            self.logits_layer = self._logits_layer(
                self.highway_layer, params["num_classes"]
            )
        ...
        with tf.name_scope("train"):
            self.loss = tf.losses.sparse_softmax_cross_entropy(
                labels=labels, logits=self.logits_layer
            )
            self.optimizer = tf.train.AdamOptimizer(params["learning_rate"]).minimize(
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            logging_hook = tf.train.LoggingTensorHook(
                {"accuracy": self.acc}, every_n_iter=100
            )
            train_op = tf.group(self.optimizer, self.acc_update_op)
            return tf.estimator.EstimatorSpec(
                mode, loss=self.loss, train_op=train_op, training_hooks=[logging_hook]
            )
        ...
        for i, (filter_size, filter_num) in enumerate(kernels):
            with tf.name_scope("conv-maxpool-%s" % filter_size):
                ...
                pooled = tf.layers.average_pooling1d(
                    inputs=conv, pool_size=int(conv.get_shape()[-2]), strides=1
                )
