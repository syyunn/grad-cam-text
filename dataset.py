...
class Word2vecEnWordEmbedder(Word2vecEmbedder):
    def __init__(self):
        model = word2vec.KeyedVectors.load_word2vec_format(
            "./word2vec/GoogleNews-vectors-negative300-SLIM.bin", binary=True
        )
...
class Dataset:
    ...
    def train_input_fn(self, batch_size, padded_size, epoch=20, shuffle=True):
        ...
        dataset = dataset.padded_batch(
            batch_size, padded_shapes=([padded_size, self.embedder.embedding_dim], [])
        )
    ...
    def predict_input_fn(self, _inputs: list, padded_size):
        ...
        dataset = dataset.padded_batch(
            len(_inputs), padded_shapes=([padded_size, self.embedder.embedding_dim], [])
        )
        iterator = dataset.make_one_shot_iterator()
        feature, label = iterator.get_next()
        return {"x": feature}
...
class SST(Dataset):
    def __init__(self, embed_cls):
        ...
        self.train_x, self.train_y = self._load_data(
            "https://raw.githubusercontent.com/HaebinShin/stanford-sentiment-dataset/master/stsa.binary.phrases.train"
        )
        self.test_x, self.test_y = self._load_data(
            "https://raw.githubusercontent.com/HaebinShin/stanford-sentiment-dataset/master/stsa.binary.test"
        )
